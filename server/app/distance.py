...
def generate_random_routes(n, area):
    """ Generate n random routes within the specified area.

    :param n: Number of routes to generate.
    :param area: Tuple of (min_lat, max_lat, min_lng, max_lng) representing the area.
    :return: List of routes in the specified format.
    """
    ...
    for _ in range(n):
        ...
        start_lng = random.uniform(min_lng, max_lng)
        end_lat = random.uniform(min_lat, max_lat)
        end_lng = random.uniform(min_lng, max_lng)

        route = {"start": [start_lat, start_lng], "end": [end_lat, end_lng]}
        routes.append(route)
...
san_francisco_area = (37.707398, 37.832266, -122.517910, -122.371002)


def match(start, end):
    DATA = generate_random_routes(1000, san_francisco_area)
    # DATA = [
